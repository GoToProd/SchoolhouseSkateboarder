...
//
//  Created by Ксения Баскакова on 04.08.2022.
//

import SpriteKit


///Эта структура содержит различные физические категории, и мы можем определить, какие типы объектов сталкиваются
///или контактируют друг с другом
...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    //Массив, содержащий все текущие секции тратуара
    var bricks = [SKSpriteNode]()
    
    //Массив, содержащий все активные алмазы
    var gems = [SKSpriteNode]()
    
    //Размер секции на тротуаре
    ...
    //Константа для гравитиации (того, как быстро объекты падают на Землю)
    ...
    //Время последнего вызова метода обновления
    var lastUpdateTime: TimeInterval?
    
    //Здесь мы добавляем нового персонажа - скейтбордистку
    ...
    override func didMove(to view: SKView) {
        ...
        let xMid = frame.midX
        let yMid = frame.midY
        background.position = CGPoint(x: xMid - 10, y: yMid - 10)
        addChild(background)
        ...
        //Добавляем распознование нажатия, чтобы знать, когда пользователь нажмет на экран
        let tapMethod = #selector(GameScene.handleTap(tapGesture:))
        let tapGesture = UITapGestureRecognizer(target: self, action: tapMethod)
        view.addGestureRecognizer(tapGesture)
        
        //Слой что бы начать играть
        ...
        func updateBricks (withScrollAmount currentScrollAmount: CGFloat) {
            ...
            for brick in bricks {
                ...
                if newX < -brickSize.width {
                    ...
                    //Обновляем значение для крайней правой секци
            ...
            while farthestRightBrickX < frame.width {
                ...
                //Время от времени мы останавливаем разрывы, через которые герой должен перепрыгнуть
                let randomNumber = arc4random_uniform(99)
                
                if randomNumber < 2 && score > 10 {
                    //2-процентнй шанс на то, что у нас возникнет разрыв между секциями,
                    //после того как игрок наберет 10 очков
                    ...
                    //В игре имеется 2-процентный шанс на изменение уровня секции
                    // после того как игрок наберет 20 очков
                    if brickLevel == .high {
                        brickLevel = .low
                    }
    ...
    //Задаем начальное положение скейтбордистки, zPosition and minimumY
    func resetSkater() {
    ...
    func setupLabels() {
        ...
        let highScoreLabel: SKLabelNode = SKLabelNode(text: "лучший результат")
    ...
    func startGame() {
        //Возвращаем к начальным условиям при запуске новой игры
        gameState = .running
        resetSkater()
        score = 0
    ...
    override func update(_ currentTime: TimeInterval) {
        ...
        //Рассчитываем, на сколько далеко должны сдвинуться объекты при данном обновлении
    ...
    //MARK:- SKPhysucsContactDelegate Methods
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        if contact.bodyA.categoryBitMask == PhysicsCategory.skater && contact.bodyB.categoryBitMask == PhysicsCategory.brick {
            ...
            //Скейтбордистка коснулась алмаза, поэтому убираем его
